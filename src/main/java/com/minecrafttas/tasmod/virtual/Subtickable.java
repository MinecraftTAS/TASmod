...
public class Subtickable<T> {
	 ...
	 * If a peripheral <em>parent</em> is updated, it first adds it's current state
	 * to the subtickList before updating.<br>
	 * This makes the subtickList a list of previous peripheral states, with the
	 * first element being the oldest change.<br>
	 * <br>
	 * To distinguish a peripheral of being a subtick or a "parent", subtickList is
	 * either null or not null respectively (see {@link #isParent()})<br>
	 */
	protected final List<T> subtickList;
	/**
	 * The way the parent/subtick relationship is set up (see
	 * {@link #subtickList}),<br>
	 * the subtickList contains all previous changes, while the parent contains the
	 * current state.<br>
	 * To achieve this and to prevent a ghost state from being added to the
	 * subtickList,<br>
	 * it is sometimes necessary to ignore the first time an addition is made to the
	 * subtickList,<br>
	...
	public List<T> getAll() {
		return ImmutableList.<T>builder().addAll(subtickList).add((T) this).build();
	}
